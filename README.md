...
9. Параметр value тесно связан с onChange, и без него в Input не получится писать, поскольку мы предостовали параметр value и никак не обрабатываем onChange, смысл - input связан конкрентно с нашим состоянием, а мы его не меняем то поэтому мы не можем менять и интерфейс 